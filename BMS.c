...
#define BMS_FAULT_CAN_ID 0x0B0

//store current and remembered faults
...
void Iter()
{
   ...
   if (current_received && pack_current_mA > OVER_CURRENT_THRESH)
    ...
    if ((latched_faults == 0) && current_received){
    // Battery is considered safe.
    // Close the shutdown circuit and allow HV to energize.
    HAL_SetSDC(true);
    }
    else{
    // A fault exists OR we do not have current data yet.
    // Open the shutdown circuit and deenergize the car.
    HAL_SetSDC(false);
    }

    // Create the 8-byte CAN message.
    // Start every byte at 0 so unused bytes contain known values.
    uint8_t fault_data[CAN_LEN] = {0};
    // Byte 0 tells other devices which faults are active RIGHT NOW.
    fault_data[0] = active_faults;
    // Byte 1 tells other devices which faults are LATCHED/remembered.
    fault_data[1] = latched_faults;
    // Send the fault status onto the CAN bus.
    HAL_SendCanMsg(BMS_FAULT_CAN_ID, fault_data);
...
void RxCan()
{
   ...
   if(id == 0x511){
    ...
    uint32_t raw_current =
            ...
            ((uint32_t)data[3] << 16) |
            ((uint32_t)data[4] << 8)  |
            ((uint32_t)data[5]);  //combining data 2 through 5 into one big number 
        ...
        current_received = true;  //received at least 1 real current measurement
    }
    else if(id == 0x1CF){
